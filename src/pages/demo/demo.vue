...
const callCloudRunFunction = async () => {
  ...
  try {
   const result = await app.callFunction({ 
      name: 'express', 
      // method: 'GET', // ICallFunctionOptions 不接受 method 属性
      // type: 'cloudrun', // ICallFunctionOptions 不接受 type 属性
      // path: '/api/users', // ICallFunctionOptions 不接受 path 属性
      // header: { 'Content-Type': 'application/json; charset=UTF-8' }, // ICallFunctionOptions 不接受 header 属性
      data: {
							key1: 'test value 1',
							key2: 'test value 2'
						},
