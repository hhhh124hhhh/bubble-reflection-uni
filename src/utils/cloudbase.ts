...
const ENV_ID: string = import.meta.env.VITE_ENV_ID || 'cloud1-9gaapyj7042812d3';
...
export const login = async () => {
  try {
    ...
    if (error.statusCode === 403) {
      console.error('登录失败: 权限错误 (403)，可能是环境配置问题或匿名登录未开启');
      // 添加降级处理：创建模拟登录状态
      console.log('启用开发环境模拟登录状态...');
      
      // 在localStorage中存储模拟的登录状态
      ...
      try {
        if (typeof localStorage !== 'undefined') {
          localStorage.setItem('cloudbase_mock_login_state', JSON.stringify(mockLoginState));
          console.log('模拟登录状态已保存到localStorage');
        }
      } catch (storageError) {
        console.warn('无法存储模拟登录状态:', storageError);
      }
...
/**
 * 获取当前登录状态（支持模拟状态）
 * @returns {Object|null} 登录状态
 */
export const getLoginState = () => {
  try {
    // 尝试获取真实登录状态
    const realState = auth.getLoginState();
    if (realState) {
      return realState;
    }
    
    // 如果没有真实登录状态，尝试获取模拟登录状态
    if (typeof localStorage !== 'undefined') {
      const mockStateStr = localStorage.getItem('cloudbase_mock_login_state');
      if (mockStateStr) {
        return JSON.parse(mockStateStr);
      }
    }
  } catch (error) {
...
/**
 * 确保用户已登录
 * @returns {Promise} 登录状态或模拟用户数据
 */
export const ensureLogin = async () => {
  // 检查环境配置
  if (!checkEnvironment()) {
    console.warn('环境ID未配置，直接启用无登录模式');
    // 不抛出错误，直接返回模拟数据
    return {
      user: {
  ...
  try {
    ...
    if (loginState && loginState.user) {
      ...
      // 如果login返回null（表示登录失败但未抛出异常）
      if (!loginResult) {
        loginState = getLoginState();
        // 再次检查登录状态，确保是否真的失败
        if (!loginState || !loginState.user) {
          console.warn('登录失败，启用无登录模式...');
          return {
            user: {
              openid: 'local_user_' + Date.now(),
              isAnonymous: true
...
export async function initCloudBase() {
  try {
    ...
    console.log('云开发初始化成功，登录状态:', loginResult?.user?.isAnonymous ? '无登录模式' : '正常登录');
    return true;
  } catch (error) {
    console.error('云开发初始化失败:', error);
    // 即使初始化失败，也尽量让应用能够运行在无登录模式
    console.warn('云开发初始化失败，应用将在有限功能下运行');
    return false;
  }
}
